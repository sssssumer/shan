...
YIUI.Grid.prototype.addGridRow = async function (rowData, rowIndex, isNewGroup) {
    ...
    if (isNewData) {
        ...
        for (let i = 0, len = this.getColumnCount(); i < len; i++) {
            ...
            let tableKey1 = dtlMetaRow.cells[i].tableKey || this.tableKey || this.metaObj.tableKey;
            let columnKey1 = dtlMetaRow.cells[i].columnKey;
            let cellKey = dtlMetaRow.cells[i].key;
            let editOpt = this.getCellEditor(cellKey).toJSON();

            if (cm.get('name') == 'rowID') { continue; }
            if (columnKey1 && form.getDocument().emptyGridRowValues) {
                const emptyRowValues1 = form.getDocument().emptyGridRowValues[tableKey1];
                value = emptyRowValues1 ? emptyRowValues1[columnKey1] ? emptyRowValues1[columnKey1]:emptyRowValues1[cellKey] : null;
            }
            if (columnKey1 == YIUI.SystemField.OID_SYS_KEY) {
                value = oid;
            } else if (columnKey1 == YIUI.SystemField.SOID_SYS_KEY) {
                value = soid;
            }
            
            rowData.data[i] = [await View.tanserDataToComponentType(editOpt, value), '', true];
            console.warn(`${dtlMetaRow.cells[i].key}[${tableKey1}.${columnKey1}] = ${rowData.data[i][0]}`);
        }
...
YIUI.Grid.prototype.focusRowChanged=function (newRowIndex, oldRowIndex) {
    var grid = this;
    grid.gridHandler.rowChange(grid, newRowIndex, oldRowIndex);
},
    // 去除所有空白行以及空白分组
    YIUI.Grid.prototype.removeAutoRowAndGroup=function () {
...
YIUI.Grid.prototype.isCellEnable=function (rowIndex, cellKey) {
...
YIUI.Grid.prototype.setCellEnable=function (rowIndex, cellKey, enable) {
    ...
    if (typeof cellKey === 'string') {
        ...
        for (var i = 0, len = colInfoes.length; i < len; i++) {
            ...
            const state = this.state.setIn(['dataModel', 'data', rowIndex, 'data', colIndex, 2], enable);
            this.changeState(state);
...
YIUI.Grid.prototype.getValueAt=function (rowIndex, colIndex) {
    if(colIndex==-1)
    {return undefined}
...
YIUI.Grid.prototype.setBkmkRow = function (rowIndex, bkmkRow) {
    this.state = this.state.setIn(['dataModel', 'data', rowIndex, 'bkmkRow'], bkmkRow);
    return this.gridHandler;
}