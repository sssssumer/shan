...
YIUI.UICalcProcess = YIUI.extend(YIUI.UICalcProcess, {
    ...
    evalFilter: async function(evalEnv,fieldKey, formula) {
        let  result, gridKey, rowIndex, colIndex, grid;
        //const scope = View.UIScopeTrees.get(this.form.formKey)[fieldKey + ".ItemFilter." + formula];
        ...
        } else if (this.form.getComponent(fieldKey).bindingCellKey.length > 0) {
            let bindingCellKey = this.form.getComponent(fieldKey).bindingCellKey;
            cellLocation = this.form.getCellLocation(bindingCellKey);
            gridKey = cellLocation.key;
            colIndex = cellLocation.column;
            rowIndex = cellLocation.row;
            if (rowIndex == null || rowIndex == -1) {
        ...
        if (scope && !scope.includeOnlyUIFunction) {
            ...
            // var cellLocation = this.form.getCellLocation(fieldKey);
            // 如果存在单元位置，那么是一个集合组件
            let bookmarks;
            if (grid) {
            ...
            const cxt = new View.Context(this.form);
            if (cellLocation) {
